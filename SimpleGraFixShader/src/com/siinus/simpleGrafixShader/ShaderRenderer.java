...
public class ShaderRenderer extends Renderer {
    ...
    private int ambientLight = 0xff3f3f3f;
    ...
    public void drawImage(Image image, int offX, int offY) {
        ...
        if (image instanceof IShaderImage) {
            this.drawShader(offX, offY, image.getWidth(), image.getHeight(), ((IShaderImage) image).getLightBlock(), (x, y) -> image.getPixels()[x + y * image.getWidth()]);
        }
    }
    ...
    public void drawImageTile(ImageTile image, int offX, int offY, int tileX, int tileY) {
        ...
        if (image instanceof IShaderImage) {
            this.drawShader(offX, offY, image.getTileWidth(), image.getTileHeight(), ((IShaderImage) image).getLightBlock(), (x, y) -> image.getPixels()[x + tileX * image.getTileWidth() + (y + tileY * image.getTileHeight()) * image.getWidth()] );
        }
    }

    protected void drawShader(int offX, int offY, int width, int height, int lightBlock,  Renderer.PixelIndexer pixelIndex) {
