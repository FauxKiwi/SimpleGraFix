...
import com.siinus.simpleGrafix.gfx.LowRamImage;
...
public class Renderer {
    ...
    protected HashMap<Double, int[]> zBuffer = new HashMap<>();
    ...
    public void drawImage(@NotNull LowRamImage image, int offX, int offY) {
        draw(offX, offY, image.getWidth(), image.getHeight(), (x, y) -> image.getPixelColor(x + y * image.getWidth()));
    }
     ...
     */
    public void draw(int offX, int offY, int width, int height, PixelIndexer pixelIndex) {
        int x1 = Math.min(offX, 0);
        int x2 = Math.min(width, this.width-offX);
        int y1 = Math.min(offY, 0);
        int y2 = Math.min(height, this.height-offY);
