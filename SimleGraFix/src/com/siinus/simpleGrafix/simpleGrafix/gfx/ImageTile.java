...
/**
 * Represents an image that can be rendered partially by this api using {@link com.siinus.simpleGrafix.Renderer#drawImageTile(ImageTile, int, int, int, int)}.<br>
 * This can be used to make animations or objects that use different image at different states.<br>
 * Also, this extends the normal {@link Image}, so it can also be drawn using {@link com.siinus.simpleGrafix.Renderer#drawImage(Image, int, int)}.
 *
 * @author Simon
 * @since 1.0
 */
